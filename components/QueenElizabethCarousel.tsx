...
const QueenElizabethCarousel: React.FC = () => {
  ...
  const images: CarouselImage[] = [
    ...
    {
      src: 'https://images.unsplash.com/photo-1547471080-7cc2caa01a7e?auto=format&fit=crop&q=80&w=1600',
    ...
    {
      src: 'https://images.unsplash.com/photo-1502219684198-f9188219994d?auto=format&fit=crop&q=80&w=1600',
  ...
  return (
    ...
    >
      <div className="carousel-container luxury-image-container rounded-sm border-2 border-[#1A1A1A] overflow-hidden shadow-2xl bg-[#1A1A1A]">
          ...
          {images.map((image, index) => (
            ...
            >
              <img
                ...
                alt={image.alt}
                className={`carousel-image w-full h-full object-cover transition-transform duration-[10s] ease-linear opacity-100 ${
              ...
              {/* Solid background for caption instead of blur to keep imagery sharp */}
              {image.caption && (
                <div className="carousel-caption absolute bottom-0 left-0 right-0 p-8 md:p-12 bg-black/60 border-t border-white/5 flex flex-col items-center justify-center">
                  <p className="text-[#F5F5DC] font-serif text-xl md:text-3xl italic font-light text-center tracking-wide leading-tight">
                    {image.caption}
                  </p>
                  <div className="w-12 h-[1px] bg-[#D4AF37] mt-6 opacity-60" />
                </div>
              )}
            </div>
          ))}
        ...
        <button 
          className="carousel-arrow carousel-arrow-left absolute top-1/2 left-6 -translate-y-1/2 z-20 w-14 h-14 rounded-full bg-white/10 border border-white/20 text-white flex items-center justify-center opacity-0 group-hover:opacity-100 transition-all duration-500 hover:bg-[#D4AF37] hover:text-[#1A1A1A] hover:scale-110 active:scale-95"
          onClick={prevSlide}
        ...
        <button 
          className="carousel-arrow carousel-arrow-right absolute top-1/2 right-6 -translate-y-1/2 z-20 w-14 h-14 rounded-full bg-white/10 border border-white/20 text-white flex items-center justify-center opacity-0 group-hover:opacity-100 transition-all duration-500 hover:bg-[#D4AF37] hover:text-[#1A1A1A] hover:scale-110 active:scale-95"
          onClick={nextSlide}
        ...
        <div className="carousel-dots absolute bottom-6 right-8 flex gap-3 z-20">
          {images.map((_, index) => (
            <button
              key={index}
